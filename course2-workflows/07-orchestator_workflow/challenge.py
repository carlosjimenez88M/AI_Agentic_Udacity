...
load_dotenv()
client = OpenAI(
    # NOTA: Si estás en Vocareum/Udacity, descomenta la siguiente línea:
...
def extract_xml(text: str, tag: str) -> str:
    ...
    CORRECCIÓN CRÍTICA: Si no encuentra etiquetas, devuelve el texto original.
    Esto evita que el reporte salga en blanco si el LLM es perezoso.
    """
    pattern = rf"<{tag}>(.*?)</{tag}>"
    match = re.search(pattern, text, re.DOTALL)
    # Si hay match, devuelve lo de adentro. Si no, devuelve TODO el texto.
...
class Orchestrator:
    ...
    def get_worker(self, task_type: str) -> WorkerAgent:
        ...
        # Lógica de despacho robusta
        if "hematology" in type_lower or "blood" in type_lower:
            ...
            # Fallback para errores o tipos desconocidos
    ...
    def process(self, task: str) -> Dict:
        ...
        # Usamos extract_xml mejorado para obtener el plan
